...
public class MinServer extends minProtocol.minService implements Coprocessor,CoprocessorService{

    private RegionCoprocessorEnvironment env;   // 定义环境

    @Override
    public void getmin(RpcController controller, minProtocol.minRequest request, RpcCallback<minProtocol.minResponse> done) {
        ...
        // 定义变量
        minProtocol.minResponse response = null;
        ...
        try {
            ...
            // 设置返回结果
            response = minProtocol.minResponse.newBuilder().setMin((long) (min!=null?min.doubleValue():Double.MIN_NORMAL)).build();
        } catch (IOException e) {
